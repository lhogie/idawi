...
public class Demo4_deploy_local_peers {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		Component initialThing = new Component(new ComponentRef("0"));
		List<Component> things = new ArrayList<>();
		var l = ComponentRef.create("c-", 50);
		...
		things.forEach(t -> System.out.println(t + " => " + t.mapService().map.outs(t.ref())));

		// install this service on the last component
		var s = new DummyService(last);
		// things.forEach(t -> t.services.add(new DummyService(t)));

		initialThing.defaultRoutingProtocol().exec(DummyService.op.class, null, new TargetComponents.Unicast(last.ref()),
				true, "hello!");
		s.wait.poll_sync();
