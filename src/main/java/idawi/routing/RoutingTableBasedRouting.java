...
public class RoutingTableBasedRouting<T extends RoutingTable> extends RoutingService {
	...
	public Collection<ComponentDescriptor> relaysTo(Set<ComponentDescriptor> to, TransportLayer protocol) {
		 ...
		 * 
		 * // if it's a broadcast message if (to == null) { // but the node is
		 * disconnected if (neighbors.isEmpty()) { return
		 * lookupService(RegistryService.class).list(); } else { return neighbors; } }
		 * // the msg is targeted to specific nodes else { // but the node is
		 * disconnected if (neighbors.isEmpty()) { // try to reach the destinations
		 * directly return to; } else { Set<ComponentDescriptor> relays = new
		 * HashSet<>(to.size());
		 * 
		 * for (ComponentDescriptor t : to) { ComponentDescriptor relay =
		 * routingTable.getRelay(t);
		 * 
